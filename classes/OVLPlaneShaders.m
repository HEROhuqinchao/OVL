...
#define SIMPLE_VERTEX @"stretch.vsh"
//#define SIMPLE_VERTEX @"simple.vsh"
...
-(void) setProjection:(GLKMatrix4*)projection {
    ...
    CGFloat ratioTexture;
    CGFloat ratioView;
    if (_fLandscape) {
        ratioTexture = _sizeTexture.height / _sizeTexture.width;
        ratioView = _sizeView.height / _sizeView.width;
        NSLog(@"PS ratio=%f",ratioView/ratioTexture);
        glUniform2f(_uRatio, 1.0, ratioView/ratioTexture);
    }else{
        ratioTexture = _sizeTexture.width / _sizeTexture.height;
        ratioView = _sizeView.width / _sizeView.height;
        NSLog(@"PS ratio=%f",ratioView/ratioTexture);
        glUniform2f(_uRatio, ratioView/ratioTexture, 1.0);
    }
    glBindBuffer(GL_ARRAY_BUFFER, _bufVertices);
